...
from mode import make_spatialVx   
from mode import feature_finder
from mode import censqdelta
from mode import centmatch
...
def deltammSqcen(x, p, max_delta, const, N, show):
    ...
    if (np.all(bigQ > max_delta)):
        ...
        robjects.r('''
           f <- function(m, n){cbind(rep(1:n, 3 * m), rep(rep(1:m, each = n), 3), rep(1:3, each = n * m), 1:(3 * n * m))}
           ''')
        ind = np.array(robjects.r['f'](m, n))
